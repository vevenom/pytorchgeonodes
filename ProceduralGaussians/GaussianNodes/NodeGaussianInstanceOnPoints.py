...
from ProceduralGaussians.GaussianNodes.PrimitiveGaussian import PrimitiveGaussian, join_cloned_verts_faces
...
class NodeInstanceOnPoints(Node):
    ...
    def forward(self, inputs_dict):
        # TODO: Requires more testing. Only used in cabinet class.
        ...
        for pg_i, pg in enumerate(instance):
            assert isinstance(pg, PrimitiveGaussian), 'Geometry must be a PrimitiveGaussian, got {}'.format(type(pg))

            verts = pg.primitive_mesh.verts
            faces = pg.primitive_mesh.faces

            assert verts.shape[0] == pg.verts.shape[0], 'Batch size > 1 not supported yet'

            rotation_i = rotation[pg_i]
            scale_i = scale[pg_i]

            # Batched version
            points_i = torch.zeros_like(points[pg_i]) - points[pg_i]
            ...
            mesh_p = join_cloned_verts_faces(v, faces, pg, 'InstanceOnPoints')

            gaussian_primitives.append(mesh_p)

        inputs_dict[self.name][NodeStrings.OUT_str + 'Instances'] = gaussian_primitives

        if not len(self.in_edges):
            self.cached_output = {NodeStrings.OUT_str + 'Instances': gaussian_primitives}
