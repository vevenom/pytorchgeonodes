            ...
            for i in range(switch_bool.shape[0]):
                if switch_bool[i]:
                    if true_values is None:
                        primitive_gaussians.append(
                            PrimitiveGaussian.create_empty(switch_bool.device))
                    else:
                        assert true_values[i].verts.shape[0] == 1, f'shape was {true_values[i].verts.shape[0]}'
                        meshes.append(true_values[i])
                else:
                    if false_values is None:
                        primitive_mesh = PrimitiveMesh('Empty', device=switch_bool.device)
                        primitive_gaussians.append(PrimitiveGaussian('Empty', primitive_mesh,
                                                                     device=switch_bool.device))
                    else:
                        if false_values[i].primitive_type != "Empty":
                            assert false_values[i].batch_size == 1, f'shape was {false_values[i].verts.shape[0]}'
                            primitive_gaussians.append(false_values[i])

            # TODO: Is it always called Output_006?
            inputs_dict[self.name][NodeStrings.OUT_str + 'Output_006'] = primitive_gaussians
