...
from ProceduralGaussians.GaussianNodes.NodeGaussianJoinGeometry import NodeGaussianJoinGeometry
from ProceduralGaussians.GaussianNodes.NodeGaussianPrimitiveCube import NodeGaussianPrimitiveCube
from ProceduralGaussians.GaussianNodes.NodeGaussianPrimitiveCylinder import NodeGaussianPrimitiveCylinder
from ProceduralGaussians.GaussianNodes.NodeGaussianRealizeInstances import NodeGaussianRealizeInstances
from ProceduralGaussians.GaussianNodes.NodeGaussianTransformGeometry import NodeGaussianTransformGeometry
...
class GeometryNodes(torch.nn.Module):
    def __init__(self, shape_program: BlenderShapeProgram, node_tree=None, name='GeometryNodes',
                 use_gaussians=False, counters=Counters(),
    ...
    def _generate_graph_(self, blender_shape_program: BlenderShapeProgram, node_tree, check_pickable=False):
        ...
        for bpy_node in node_tree.nodes:
            if self.use_gaussians:
                torch_node = self.create_gaussian_node(bpy_node, blender_shape_program)
            else:
    ...
    def create_gaussian_node(self, bpy_node, blender_shape_program: BlenderShapeProgram):
        new_node = None  # type: Node
        if bpy_node.type == NodeTypes.MATH_str:
            new_node = NodeMath(bpy_node, config=self.config)
        elif bpy_node.type == NodeTypes.COMBXYZ_str:
        ...
        #     new_node = NodePoints(bpy_node)
            ...
            new_node = NodeGaussianPrimitiveCube(self.counters.total_number_base_primitives, bpy_node,
                                                 config=self.config)
            self.counters.increment_total_number_base_primitives()
        elif bpy_node.type == NodeTypes.MESH_PRIMITIVE_CYLINDER_str:
            new_node = NodeGaussianPrimitiveCylinder(self.counters.total_number_base_primitives, bpy_node,
                                                     config=self.config)
            self.counters.increment_total_number_base_primitives()
        elif bpy_node.type == NodeTypes.INSTANCE_ON_POINTS_str:
            new_node = NodeInstanceOnPoints(bpy_node, config=self.config)
        elif bpy_node.type == NodeTypes.REALIZE_INSTANCES_str:
            new_node = NodeGaussianRealizeInstances(bpy_node, config=self.config)
        elif bpy_node.type == NodeTypes.TRANSFORM_GEOMETRY_str:
            new_node = NodeGaussianTransformGeometry(bpy_node, config=self.config)
        elif bpy_node.type == NodeTypes.VECT_MATH_str:
            new_node = NodeVectMath(bpy_node, config=self.config)
        elif bpy_node.type == NodeTypes.GROUP_str:
            # create a new graph for the group
            new_graph = GeometryNodes(blender_shape_program, node_tree=bpy_node.node_tree, name=bpy_node.name,
                                      use_gaussians=True, config_path=self.config_path)
            new_node = NodeGroup(bpy_node, new_graph, config=self.config)
        elif bpy_node.type == NodeTypes.JOIN_GEOMETRY_str:
            new_node = NodeGaussianJoinGeometry(bpy_node, config=self.config)
        elif bpy_node.type == NodeTypes.SWITCH_str:
            new_node = NodeSwitch(bpy_node, config=self.config)
        elif bpy_node.type == NodeTypes.FRAME_str:
            # Frames seem to be used only for better visualization in Blender to seperate different blocks of nodes
        ...
        if transform2blender_coords and not self.is_group_node:
            if not self.use_gaussians:
                ...
                for output in outputs:
                    for out in output:
                        for o_i, o in enumerate(out):
                            o = o.transform(self.transform_py3d2blender)
                            out[o_i] = o
    ...
    def save_trainable_params(self, path):
        ...
        trainable_params = self.state_dict()
    ...
    def visualize_gaussians(self, outputs, b_index=0):
        '''
        Visualize the gaussian outputs.
        '''
        outputs_o3d = []
        for output in outputs:
            output = output[b_index]
            for gaussian_primitive in output:
                means, scales, quats, colors, opacities  = gaussian_primitive.get_gaussians()
                geometries = []
                for i in range(means.shape[1]):
                    ellipsoid = create_ellipsoid(means[:, i], scales[:, i], quats[:, i])
                    geometries.append(ellipsoid)
            outputs_o3d.append(geometries)
        return outputs_o3d
...
def create_ellipsoid(mean, scale, quat):
    mean = mean.detach().cpu().numpy().squeeze()
    scale = scale.detach().cpu().numpy().squeeze()
    quat = quat.detach().cpu().numpy().squeeze()
    ...
    scaling_matrix[0, 0] = scale[0].item()
    scaling_matrix[1, 1] = scale[1].item()
    scaling_matrix[2, 2] = scale[2].item()
    ...
    transform_py3d2blender = np.linalg.inv(transform_py3d2blender)
    transform_py3d2blender = torch.tensor(transform_py3d2blender, dtype=torch.float32)
    transform_py3d2blender = Transform3d(matrix=transform_py3d2blender)

    gauss_rot_mat = quaternion_to_matrix(torch.from_numpy(quat))

    rot_mat = quaternion_to_matrix(torch.from_numpy(quat)).numpy() #gauss_rot_mat.numpy() #quaternion_to_matrix(torch.from_numpy(quat)).numpy()

    transform_matrix = np.eye(4)
    transform_matrix[:3, :3] = gauss_rot_mat @ scaling_matrix[:3, :3]
