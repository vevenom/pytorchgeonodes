...
import torch
from pytorch3d.loss.chamfer import _validate_chamfer_reduction_inputs, _handle_pointcloud_input, chamfer_distance
...
class LossBatched(torch.nn.Module):
    def __init__(self, settings):
        ...
        self.cd_clamp = settings.cd_clamp
        self.fine_loss_thresh = settings.fine_loss_thresh
        self.render_loss_constant = settings.render_loss_constant
    ...
    def forward(self, obj_mesh, scene_dict):
        ...
        # chamfer_loss = one_way_chamfer_distance(target_obj_pcd, mesh_pcd, clamp_max=self.cd_clamp)
        chamfer_loss, _ = chamfer_distance(target_obj_pcd, mesh_pcd)
        ...
        cd_loss = 1.0 * cd_loss_x.mean()  + 1.0 * cd_loss_y.mean()

        return cd_loss

        # chamfer_loss = chamfer_loss * 10

        # if chamfer_loss > self.fine_loss_thresh:
        #     total_loss_randc_loss = self.render_loss_constant
        # else:
        ...
        #
        # total_loss = chamfer_loss + total_loss_randc_loss
        #
        # return total_loss


    def calculate_loss(self, mask_pred, mask_gt, depth_final, depth_sensor,
        ...
        # plt.subplot(142)
        # plt.imshow(depth_final[0].detach().cpu().numpy())
        # plt.subplot(143)
        # plt.imshow(depth_sensor[0].detach().cpu().numpy())
        # plt.subplot(144)
        # plt.imshow(mask_depth_valid_render_pred[0].detach().cpu().numpy())
        # plt.show()
