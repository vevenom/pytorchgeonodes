...
class SubdivideMeshesCustom(nn.Module):
    """
    ...
    def __init__(self, meshes=None):
        ...
        """
        super(SubdivideMeshesCustom, self).__init__()
    ...
    def subdivide_homogeneous(self, meshes, feats=None):
        ...
        if feats is not None:
            ...
            # Take 0 index of the features at the vertices that form each edge.
            new_feats = feats[:, edges][:,:,0]
    ...
    def subdivide_heterogenerous(self, meshes, feats=None):
        ...
        with torch.no_grad():
            ...
            print("verts_ordered_idx", verts_ordered_idx)
        ...
        if feats is not None:
            ...
            #     # feats is in packed format, transform it from packed to
            #     # padded, i.e. (N*V, D) to (N, V, D).
            #     feats = feats.view(verts.size(0), verts.size(1), 3)
            # if feats.dim() != 3:
            #     raise ValueError("features need to be of shape (N, V, D) or (N*V, D)")

            # Take mean of the features at the vertices that form each edge.
        ...
        print("verts_list", verts_list[0].shape, verts_list[1].shape)
        # assert False
        ...
        if feats is None:
            ...
            # new_feats = torch.stack(feats_list, dim=0)
            feats_list = list(feats_list)
            return new_meshes, feats_list
