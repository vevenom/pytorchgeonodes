...
class MeshRendererScannet(nn.Module):
    ...
    def forward(self, meshes_world, **kwargs) -> torch.Tensor:
        ...
        #meshes_world._compute_vertex_normals()
        #meshes_world._compute_face_areas_normals()

        #import time
        #verts = meshes_world.verts_packed()  # (V, 3)
        #faces = meshes_world.faces_packed()  # (F, 3)
        #vertex_normals = meshes_world.verts_normals_packed()  # (V, 3)
        #faces_verts = verts[faces]
        #faces_normals = vertex_normals[faces]
        #faces_normals_ = meshes_world.faces_normals_packed()
        #start_time = time.time()
        ...
        # this looks more smooth, if pixelnormals are used take this one
        #pixel_normals = interpolate_face_attributes(
        #    fragments.pix_to_face, fragments.bary_coords, faces_normals
        #)

        #todo version below did not work because of wrong CAD faces
        #pixel_normals = interpolate_face_attributes(
        #    fragments.pix_to_face, ones, faces_normals
        #)

        #return images, fragments, pixel_normals

        return fragments # , pixel_normals

class MeshRendererViewSelection(nn.Module):
    """
    A class for rendering a batch of heterogeneous meshes. The class should
    be initialized with a rasterizer and shader class which each have a forward
    function.
